...
async function build() {
  try {
    ...
    const displayTabTotalResult = await esbuild.build({
      entryPoints: ["src/ui/displayTabTotal.ts"],
      bundle: true,
      write: false,
      target: "es2017",
      format: "iife",
    ...
    const uiModulesCode =
      createIconResult.outputFiles[0].text +
      "\n" +
      ...
      "\n" +
      displayTabTotalResult.outputFiles[0].text +
      "\n" +
    ...
    const cssContent = fs.readFileSync(
      path.resolve(__dirname, "src/ui/ui.css"),
      "utf8"
    );
    let htmlContent = fs.readFileSync(
      path.resolve(__dirname, "src/ui/ui.html"),
      "utf8"
    );
