      ...
      if (node.type === 'COMPONENT_SET') {
        ...
        if ('children' in node) {
          for (const child of node.children) {
            const childResults = await processNodeComponent(child, componentsResult); // Рекурсивный вызов
            if (childResults) {
...
export const processComponentSetNode = async (node: ComponentSetNode, parentSet: ComponentSetNode | null = null): Promise<Partial<ComponentData> | null> => {
